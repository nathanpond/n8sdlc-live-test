...
1. Zero runtime dependencies (dev dependencies are fine). — **test-enforced** (guard test asserts `package.json` `dependencies` is empty; guard story #5, M0)
2. Warnings and lint findings are errors in CI (`eslint --max-warnings 0`, `tsc` strict). — **test-enforced** (build setting in the CI workflow; story #4, M0)
3. The storage file format is versioned JSON — never break reading an older file. — **test-enforced** (`test/storage-compat.test.ts` loads every frozen fixture in `test/fixtures/storage/` through the production `loadStore`; landed with story #12. A format change must add a fixture, never edit one.)
